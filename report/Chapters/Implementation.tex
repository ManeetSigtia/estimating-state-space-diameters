...
Instead of loading and retaining the entire text corpus in memory as a monolithic list of strings, the \texttt{SASParser} class was implemented as a Finite State Machine (FSM) driven by a Python \texttt{Iterator[str]}. By utilizing the native \texttt{iter()} function, the parser consumed the string payload sequentially.

\begin{lstlisting}[language=Python, caption={Lazy-loading FSM loop in SASParser.}, label={lst:lazyload}]
...
The exact Floyd-Warshall algorithm computes a dense $|V| \times |V|$ distance matrix, incurring $O(|V|^2)$ spatial complexity regardless of graph sparsity. While computationally manageable for small state spaces, this memory requirement scales quadratically with the number of reachable states and triggers rapid exhaustion of available RAM on standard hardware. To quantify this risk concretely: the benchmark results indicated that Floyd-Warshall completed in approximately 2 seconds for \texttt{blocksworld\_4} ($|V| = 125$), and approximately 648 seconds for \texttt{blocksworld\_5} ($|V| = 866$). Projecting this $O(|V|^3)$ scaling to the 1{,}000-node Erdős-Rényi graphs — $(1000 / 866)^3 \approx 1.54$ — suggested a per-round runtime exceeding 1{,}000 seconds, or approximately 83 minutes of Floyd-Warshall execution across five rounds for a single file. To prevent such Out-Of-Memory (OOM) operating system crashes during unsupervised benchmarking, the pipeline implemented a strict defensive programming threshold: \texttt{FW\_NODE\_LIMIT = 999}. This value was selected to admit all graphs with $|V| \leq 999$ — thereby including \texttt{blocksworld\_5} at 866 nodes — while excluding the 1{,}000-node and larger Erdős-Rényi instances for which projected runtimes rendered inclusion impractical.
