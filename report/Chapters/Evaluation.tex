...
The primary objective of this evaluation is to benchmark the performance of two exact baseline methods --- the $O(|V|^3)$ Floyd-Warshall algorithm and the exact $O(|V|(|V| + |E|))$ NetworkX All-Pairs Shortest Path implementation --- against the $\tilde{O}(m\sqrt{n})$ Aingworth approximation \cite{aingworth96}. To provide a comprehensive assessment, the algorithms were measured across three primary dimensions:
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/fig1_fw_vs_networkx.pdf}
    \caption{Mean execution time (log scale) versus node count for Floyd-Warshall and NetworkX Exact, evaluated on the three Floyd-Warshall-eligible instances. The cubic growth of Floyd-Warshall is clearly visible against the near-quadratic scaling of the NetworkX BFS baseline.}
    \label{fig:fw_vs_nx}
\end{figure}
...
NetworkX executes a BFS (or Dijkstra's algorithm for weighted graphs) from every vertex. For unweighted graphs, a single BFS resolves in $O(|V| + |E|)$, yielding a total complexity of $O(|V|(|V| + |E|))$. The critical factor is sparsity. Automated planning state spaces and scale-free networks are structurally sparse: the edge count $|E|$ is proportionally far closer to $O(|V|)$ than to the maximum $O(|V|^2)$. By exploiting this sparsity, the NetworkX complexity effectively collapses toward $O(|V|^2)$ on these specific topologies --- an entire polynomial degree better than Floyd-Warshall.
...
Furthermore, NetworkX delegates its core operations --- dictionary hash-table probing, priority queue insertion and extraction in Dijkstra's algorithm, and BFS queue management --- to pre-compiled C extensions (CPython built-ins) and Cython-compiled routines within the NetworkX library itself. C-compiled code avoids the bytecode interpretation overhead of the CPython runtime, eliminates Python object allocation per loop iteration, and enables the compiler to apply low-level optimisations such as loop unrolling and SIMD vectorisation where applicable. Each of these mechanisms contributes a constant-factor speedup over equivalent pure-Python implementations. Collectively, NetworkX outperforms Floyd-Warshall not merely by executing fewer theoretical operations, but by operating in mechanical sympathy with the host machine's memory hierarchy and instruction pipeline.
...
An interesting secondary observation concerns the Erdős-Rényi graphs. Despite scaling from 1,000 to 10,000 nodes, the true diameter remains either 6 or 7 across all three instances. This reflects the well-established small-world property of Erdős-Rényi random graphs with supercritical connectivity: as node count grows, the diameter increases only logarithmically rather than proportionally.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/fig2_algorithm_comparison.pdf}
    \caption{Mean execution times for Exact (NetworkX), Aingworth-Default, and Aingworth-$s$=10 across all instances with $|V| \geq 3{,}000$, sorted by node count. Note the log scale on the time axis. The visitall\_3x4 bars extend substantially beyond all others, illustrating the grid topology anomaly.}
    \label{fig:algo_comparison}
\end{figure}

\subsection{The Resilience of the Greedy Set Cover: The Breaker Graph}

As described in Section 5.2.4, the \texttt{aingworth\_breaker} graph was engineered to bias the landmark node $w$ toward the graph's geographic centre, theoretically preventing full BFS traversals from being anchored at the true diameter endpoints. Theory predicted underestimation; practice refuted it.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/fig3_parameter_tuning.pdf}
    \caption{Mean execution times for Aingworth-Default versus Aingworth-$s$=10 across all instances. Instances where $s$=10 introduces an accuracy error are annotated. Instances where $s$=10 is slower than the default despite achieving accuracy are highlighted, illustrating the dominating-set inflation effect.}
...
\begin{itemize}
    \item \textbf{Modularity and Decoupling:} Confirmed. The \texttt{DiameterCalculator} processed both directed planning graphs and undirected social and random network files without any code modification, validating the architectural separation established in Chapter 3.

    \item \textbf{Memory Safety:} Confirmed. The 999-node Floyd-Warshall cutoff prevented OOM failures throughout all benchmarking rounds. The empirical FW scaling between \texttt{blocksworld\_4} and \texttt{blocksworld\_5} closely matches the theoretically predicted cubic factor, directly validating the necessity of the threshold.
\end{itemize}
